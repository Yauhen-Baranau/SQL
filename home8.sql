...
INSERT students (class_id, first_name, last_name, phone, birth_date) VALUES (3, "Диана", "Шабан", "+375245654433", "1990-04-06");
...
INSERT activities_List (activity_name) VALUES ("Математика");
INSERT activities_List (activity_name) VALUES ("Информатика");
INSERT activities_List (activity_name) VALUES ("Английский язык");
...
INSERT grade_book ( student_id, activity_id, grade) VALUES (
(SELECT id FROM students WHERE first_name = "Ева"),
(SELECT id FROM activities_List WHERE activity_name = "Информатика"),
10
);

-- SELECT с использованием подзапроса
SELECT (SELECT id FROM students WHERE first_name = "Андрей") AS student_id;

-- SELECT с использованием подзапроса
SELECT last_name
FROM students
WHERE class_id IN (SELECT class_id FROM students WHERE class_id > 2);

-- UPDATE  с использованием подзапроса, меняем оценку студенту
UPDATE grade_book
SET grade = 10
WHERE student_id = (SELECT id FROM STUDENTS WHERE last_name="Гурская");


-- DELETE с позапрсоом, удаление всех оценок по информатики
DELETE FROM grade_book
WHERE activity_id IN (SELECT id FROM activities_List WHERE activity_name="Информатика");


-- NOT EXISTS Нашли всех учеников кому не проставлены оценки
SELECT * FROM students
WHERE NOT EXISTS (SELECT * FROM grade_book WHERE student_id = students.id);
